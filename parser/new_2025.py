...
існуючу логіку з new_format (включно з фіксом is_aggregate для
"Угіддя резерву" — підхоплюється автоматично через імпорт).
"""
...
FILES_BY_YEAR = {
    2025: r"C:\projects\hunting-volyn\data\Річний звіт 2025  рік Волинська область  зі змінами.xlsx",
}
...
if __name__ == "__main__":
    ...
    print("=== Парсинг формату 2025 ===\n")

    hosts_meta_2025, finances_2025 = [], []
    populations_2025, harvest_2025, relocation_events_2025 = [], [], []

    for year, filepath in FILES_BY_YEAR.items():
        print(f"{year}: {filepath}")
        hm, fin, pop, harv, reloc = parse_new_2025(filepath, year)
        hosts_meta_2025.append(hm)
        finances_2025.append(fin)
        populations_2025.append(pop)
        harvest_2025.append(harv)
        relocation_events_2025.append(reloc)

    pd.concat(hosts_meta_2025, ignore_index=True).to_csv(PROC / "hosts_meta_2025.csv", index=False)
    pd.concat(finances_2025, ignore_index=True).to_csv(PROC / "finances_2025.csv", index=False)
    pd.concat(populations_2025, ignore_index=True).to_csv(PROC / "populations_2025.csv", index=False)
    pd.concat(harvest_2025, ignore_index=True).to_csv(PROC / "harvest_2025.csv", index=False)
    pd.concat(relocation_events_2025, ignore_index=True).to_csv(PROC / "relocation_events_2025.csv", index=False)

    print("\nЗбережено в data/processed/: hosts_meta_2025.csv, finances_2025.csv, populations_2025.csv, harvest_2025.csv, relocation_events_2025.csv")
